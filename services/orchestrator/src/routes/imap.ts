...
const createAccountSchema = z.object({
  ...
  smtpUsername: z.string().trim().min(1).max(500).optional(),
  smtpPassword: z.string().min(1).max(4_000).optional(),
  smtpFrom: z.string().trim().email().max(500).optional(),
...
const updateAccountSchema = z.object({
  ...
  smtpUsername: z.string().trim().min(1).max(500).nullable().optional(),
  smtpPassword: z.string().min(1).max(4_000).nullable().optional(),
  smtpFrom: z.string().trim().email().max(500).nullable().optional(),
    ...
    asyncHandler(async (req, res) => {
      ...
      const account = await repos.emails
        .createAccount({
          ...
          smtpPasswordEnc: input.smtpPassword ? encryptSecret(input.smtpPassword, masterKey) : null,
  ...
  router.patch(
    ...
    asyncHandler(async (req, res) => {
      ...
      const account = await repos.emails.updateAccount(id, {
        ...
        ...(input.smtpPassword !== undefined ? { smtpPasswordEnc: input.smtpPassword ? encryptSecret(input.smtpPassword, masterKey) : null } : {}),
...
function publicAccount(
  ...
  return {
    ...
    hasSmtpPassword: Boolean(account.smtpPasswordEnc),
